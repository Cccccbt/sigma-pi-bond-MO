...
function smoothRange(start, end, value) {
  ...
  return easeInOut(amount);
}

function effectiveDistance() {
  const startDistance = Math.max(5.2, state.distance + 2.4);
...
function addAtomicShell(center, phaseSign) {
  ...
  const positivePair = [positiveLobe, positiveWire];
  const negativePair = [negativeLobe, negativeWire];
  const lobes = [positiveLobe, negativeLobe, positiveWire, negativeWire];
  const cancellation =
    state.mode === "antibonding" ? smoothRange(0.16, 0.82, easeInOut(state.formation)) : 0;

  if (state.orbital === "px") {
    lobes.forEach((lobe) => lobe.scale.set(1.35, 0.78, 0.78));
    positiveLobe.position.set(center + phaseSign * 0.52, 0, 0);
    positiveWire.position.copy(positiveLobe.position);
    negativeLobe.position.set(center - phaseSign * 0.52, 0, 0);
    negativeWire.position.copy(negativeLobe.position);

    const innerPair =
      Math.abs(positiveLobe.position.x) < Math.abs(negativeLobe.position.x) ? positivePair : negativePair;
    innerPair.forEach((lobe) => {
      lobe.scale.x *= 1 - cancellation * 0.64;
      lobe.scale.y *= 1 - cancellation * 0.42;
      lobe.scale.z *= 1 - cancellation * 0.42;
    });
  } else {
    lobes.forEach((lobe) => lobe.scale.set(0.78, 1.35, 0.78));
    positiveLobe.position.set(center, phaseSign * 0.52, 0);
    positiveWire.position.copy(positiveLobe.position);
    negativeLobe.position.set(center, -phaseSign * 0.52, 0);
    negativeWire.position.copy(negativeLobe.position);
    lobes.forEach((lobe) => {
      lobe.scale.x *= 1 - cancellation * 0.46;
    });
  }
...
function makeMOLobes(distance) {
  ...
  const morph = smoothRange(0.12, 0.9, progress);
  const growth = 0.32 + morph * 0.68;
  const targetDistance = state.distance;
  moLobeGroup.visible = showMOLobesEl.checked && progress > 0.08;

  if (state.orbital === "px") {
    if (state.mode === "bonding") {
      addMOLobe(
        new THREE.Vector3(0, 0, 0),
        new THREE.Vector3((targetDistance * 0.74 + 0.9) * growth, 0.82 * growth, 0.82 * growth),
        moPositiveMaterial,
      );
      const endX = distance / 2 + 0.48;
      const endScale = new THREE.Vector3(0.62 * growth, 0.5 * growth, 0.5 * growth);
      addMOLobe(new THREE.Vector3(-endX, 0, 0), endScale, moNegativeMaterial);
      addMOLobe(new THREE.Vector3(endX, 0, 0), endScale, moNegativeMaterial);
    } else {
      const outerX = distance / 2 + 0.46;
      const innerX = Math.max(0.34, distance / 2 - 0.4);
      const outerScale = new THREE.Vector3(1.2 * growth, 0.78 * growth, 0.78 * growth);
      const innerScale = new THREE.Vector3(0.56 * growth, 0.48 * growth, 0.48 * growth);
      addMOLobe(
        new THREE.Vector3(-outerX, 0, 0),
        outerScale,
        moNegativeMaterial,
      );
      addMOLobe(
      ...
      addMOLobe(new THREE.Vector3(-innerX, 0, 0), innerScale, moPositiveMaterial);
      addMOLobe(new THREE.Vector3(innerX, 0, 0), innerScale, moNegativeMaterial);
    }
    return;
  }
  ...
  if (state.mode === "bonding") {
    addMOLobe(
      new THREE.Vector3(0, 0.58 * growth, 0),
      new THREE.Vector3((targetDistance * 0.7 + 0.78) * growth, 0.72 * growth, 0.86 * growth),
      moPositiveMaterial,
    );
    addMOLobe(
      new THREE.Vector3(0, -0.58 * growth, 0),
    ...
    const x = distance / 2;
    const y = 0.58 * growth;
    const scale = new THREE.Vector3(0.76 * growth, 1.18 * growth, 0.76 * growth);
...
function updateFormationAppearance() {
  const progress = easeInOut(state.formation);
  const aoFade = 1 - smoothRange(0.32, 0.88, progress);
  const moFade = smoothRange(0.08, 0.72, progress);

  const nodeFade = smoothRange(0.42, 0.88, progress);

  atomicPositiveMaterial.opacity = 0.4 * aoFade;
  atomicNegativeMaterial.opacity = 0.4 * aoFade;
  atomicMaterial.opacity = 0.2 * aoFade;
  moPositiveMaterial.opacity = 0.46 * moFade;
  moNegativeMaterial.opacity = 0.46 * moFade;
  moWireMaterial.opacity = 0.16 * moFade;
  nodeMaterial.opacity = 0.22 * nodeFade;
  bondMaterial.opacity = 0.38 * smoothRange(0.38, 0.92, progress);
  bond.visible = progress > 0.36;
  atomicGroup.visible = showAtomicEl.checked && aoFade > 0.015;
  nodeGroup.visible = showNodeEl.checked && nodeFade > 0.015;
}
...
function updateScene() {
  ...
  const rightBasePhase = state.orbital === "px" ? -1 : 1;
  const combinationPhase = state.mode === "bonding" ? 1 : -1;
  addAtomicShell(half, rightBasePhase * combinationPhase);
  makeMOLobes(currentDistance);
  updateFormationAppearance();
  makeNodePlanes();
